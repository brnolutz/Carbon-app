...
// ── ExerciseDB — GIFs anatômicos com músculos destacados ─────
const RAPIDAPI_KEY = "863ed60540msha869e2cf0791bd5p1ccf0cjsn2febf3210d99";
const EXERCISEDB_HOST = "exercisedb.p.rapidapi.com";

// Mapa PT-BR → ID do ExerciseDB
const EXERCISEDB_IDS = {
  "Supino (Barra)": "0001",
  "Supino Declinado (Halter)": "0076",
  "Supino Inclinado (Halter)": "0090",
  "Crucifixo na Polia (Máquina)": "1191",
  "Levantamento Terra (Barra)": "0014",
  "Barra Fixa": "0026",
  "Remada Inclinada Apoiada No Peito (Halter)": "0107",
  "Puxada Alta na Polia (Máquina)": "0019",
  "Levantamento Terra Romeno (Barra)": "0038",
  "Desenvolvimento (Halter)": "0092",
  "Elevação Lateral (Halter)": "0079",
...
// Cache em memória para não repetir chamadas à API
const _gifCache = {};

// Busca o GIF do ExerciseDB e cacheia no Supabase para economizar requisições
async function fetchExerciseGif(exName){
  // 1. Checar cache em memória
  if(_gifCache[exName]) return _gifCache[exName];

  // 2. Checar cache no Supabase
  try{
    const{data}=await supabase.from('exercise_gifs').select('gif_url').eq('exercise_name',exName).maybeSingle();
    if(data?.gif_url){
      _gifCache[exName]=data.gif_url;
      return data.gif_url;
    }
  }catch(e){}

  // 3. Buscar na API do ExerciseDB
  const id=EXERCISEDB_IDS[exName];
  if(!id) return null;
  try{
    ...
    // 4. Salvar no Supabase para não precisar chamar API de novo
    _gifCache[exName]=gifUrl;
    try{
      const{data:userData}=await supabase.auth.getUser();
      const uid=userData?.user?.id;
      if(uid){
        await supabase.from('exercise_gifs').upsert({exercise_name:exName,gif_url:gifUrl},{onConflict:'exercise_name'});
      }
    }catch(e){}
    return gifUrl;
  }catch(e){
...
// Componente de GIF anatômico — igual ao Hevy
function ExerciseSlideshow({exName,color}){
  ...
  useEffect(()=>{
    ...
    setLoading(true);
    setPaused(false);
    fetchExerciseGif(exName).then(url=>{
      setGifUrl(url);
      setLoading(false);
    });
  },[exName]);
  ...
  if(loading) return(
    <div style={{background:"#fff",borderRadius:16,minHeight:220,display:"flex",alignItems:"center",justifyContent:"center",margin:"16px 0",border:"1px solid "+color+"22"}}>
  ...
  if(!gifUrl) return(
    <div style={{background:"#f8f8f8",borderRadius:16,minHeight:180,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",margin:"16px 0",border:"1px solid rgba(0,0,0,0.06)",gap:8}}>
  ...
  return(
    <div style={{position:"relative",margin:"16px 0",borderRadius:16,overflow:"hidden",background:"#fff",border:"1px solid rgba(0,0,0,0.06)"}}>
      <img
        src={paused?undefined:gifUrl}
        alt={exName}
        style={{width:"100%",maxHeight:300,objectFit:"contain",display:"block"}}
        onError={()=>setGifUrl(null)}
      />
      {/* Se pausado mostra primeiro frame estático */}
      {paused&&<img
        src={gifUrl+"&paused=1"}
        alt={exName}
        style={{width:"100%",maxHeight:300,objectFit:"contain",display:"block",position:"absolute",top:0,left:0}}
        onError={()=>{}}
      />}
      {/* Botão pause/play */}
      <button
